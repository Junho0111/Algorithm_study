package BaekJoon.Binary_search;

import java.util.Scanner;

public class BJ1300 {
    static int N;
    static int K;

    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        N = sc.nextInt();
        K = sc.nextInt();

        System.out.println(binarySearch(K));
    }

    static int binarySearch(int end) {
        int answer = 0;
        int start = 1;

        while(start <= end) {
            int mid = (start + end) / 2;
            int sum = 0;

            for(int i = 1; i <= N; i++) {
                sum += Math.min(mid/i, N);
            }

            if(sum < K) {
                start = mid + 1;
            } else {
                answer = mid;
                end = mid - 1;
            }
        }

        return answer;
    }
}

//세준이는 크기가 N×N인 배열 A를 만들었다.
// 배열에 들어있는 수 A[i][j] = i×j 이다.
// 이 수를 일차원 배열 B에 넣으면 B의 크기는 N×N이 된다.
// B를 오름차순 정렬했을 때, B[k]를 구해보자.
//
//배열 A와 B의 인덱스는 1부터 시작한다.

//입력
//첫째 줄에 배열의 크기 N이 주어진다. N은 10^5보다 작거나 같은 자연수이다. 둘째 줄에 k가 주어진다. k는 min(10^9, N^2)보다 작거나 같은 자연수이다.

//출력
//B[k]를 출력한다.

//3
//7

//6


// k번째 원소를 구한다 == k번째 원소보다 작거나 같은수는 k-1개이다
// min(중간값(<== 1+K/2한 수)/행, 행)각 행의 개수를 더함 N은 mid값이 행을 초과할때 어차피 몫크기대로 들어가기떄문
// ex) 크기가 7x7인데 k가 41이여서 mid가 21인경우는 몫이 아니라 행만큼(n) 들어가야함.