...
public class BJ1939 {
    ...
    public static void main(String[] args) {
        ...
        for (int m = 0; m < M; m++) {
            ...
            graph[fac1].add(new Leg(fac2, weight));
            graph[fac2].add(new Leg(fac1, weight));
        ...
        while (left <= right) {
            ...
            if (bfs(start, end, mid)) {
                ...
                //왼쪽은 이미 이것보다는 무조건 큰 상태이므로 오른쪽을 야금야금 줄여서 최적의 중량을 찾는거임
                right = mid - 1;
            }
        }
    ...
    static class Leg {
        ...
        public Leg(int next, int weight) {
            this.next = next;
            this.weight = weight;
        }
    }
    ...
    private static boolean bfs(int start, int end, int weight) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (Leg next : graph[fac]) {
                if (!visited[next.next] && next.weight >= weight) {
