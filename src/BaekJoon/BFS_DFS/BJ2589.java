...
public class BJ2589 {
    ...
    static String[][] map;
    static int[] dx = new int[]{-1, 1, 0, 0}; // 위 아 왼 오
    ...
    private static void bfs(int n, int m) {
        ...
        ArrayDeque<Place>  queue = new ArrayDeque<Place>();

        queue.add(new Place(n, m, 0));
        visited[n][m] = true;

        while(!queue.isEmpty()) {
            Place place = queue.poll();

            maxHour = Math.max(maxHour, place.time);

            for(int i = 0; i < 4; i++) {
                int nextX = place.x + dx[i];
                int nextY = place.y + dy[i];


                if(nextX >= 0 && nextY >= 0 && nextX < N && nextY < M && !visited[nextX][nextY] && !map[nextX][nextY].equals("W") ) {
                    visited[nextX][nextY] = true;
                    queue.add(new Place(nextX, nextY, place.time + 1));
                }
            }
        }
    }
